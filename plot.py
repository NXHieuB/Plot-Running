...
def plot_average_with_error(interp_data_cycle, split_cycle, title1, save_folder, subfolder_name_std):
    ...
    # Fill between for standard deviation visualization and lineplot for averages
    plt.fill_between(x_left, np.min(interp_data_cycle[0], axis=0), np.max(interp_data_cycle[0], axis=0), color='pink', alpha=0.4)
    sns.lineplot(x=x_left, y=average_left, color='red', label='Mean left')
    plt.fill_between(x_right, np.min(interp_data_cycle[1], axis=0), np.max(interp_data_cycle[1], axis=0), color='lightblue', alpha=0.4)
    sns.lineplot(x=x_right, y=average_right, color='blue', label='Mean right')
  ...
  # Plot the split mode
    ...
    plt.xlabel('Running Cycle',labelpad=22, fontsize = 10)
    plt.ylabel('Angle (degree)', fontsize = 10)
    plt.title(title1)
    
    # Adjusting the ylim to ensure there is space for the phase labels below x-axis labels
    plt.ylim(plt.ylim()[0], plt.ylim()[1] * 1.1)
    
    # Setting phase labels just below the x-axis
    y_offset = plt.ylim()[0] - (plt.ylim()[1] - plt.ylim()[0]) * 0.08  # Adjust as needed
    for i, label in enumerate(phase_labels):
        if i < len(average_split_cycle_left):
            midpoint = (average_split_cycle_left[i] + (average_split_cycle_left[i-1] if i > 0 else 0)) / 2
        else:
            midpoint = (average_split_cycle_left[-1] + 100) / 2  # Assuming 100 is the end
        plt.text(midpoint, y_offset, label, ha='center', color='red')
    
    plt.legend()

    # Creating save folder and subfolder if they do not exist
    subfolder_path = os.path.join(save_folder, subfolder_name_std)
    if not os.path.exists(subfolder_path):
        os.makedirs(subfolder_path)
    
    # Save the plot
    filename = os.path.join(subfolder_path, title1.replace("/", "_") + ".png")
    ...
    while index < len(list_key) -1 :
        ...
        if key_left[1:]== "PelvisAngles" :
            for i in range(len(All_data[list_key[index]])-1) :
                if i==0:
                    title= key_left[1:] +" Tilt"
                elif i ==1 :
                    title=key_left[1:]+" Obliquity"
                elif i==2 :
                    title=key_left[1:]+ " Rotation"
            ...
            for i in range(len(All_data[list_key[index]])-1) :
                if i==0:
                    title= key_left[1:] +" Dorsi/Plantar Flexion"
                elif i ==1 :
                    title=key_left[1:]+" Abduction/Adduction"
                elif i==2 :
                    title=key_left[1:]+ " Rotation"
                left_leg= interp_shampe_many_data(All_data[key_left][i])
                right_leg=interp_shampe_many_data(All_data[key_right][i])
            ...
            for i in range(len(All_data[list_key[index]])-1) :
                if i==0:
                    title= key_left[1:] +" Flexion/Extension"
                elif i ==1 :
                    title=key_left[1:]+" Abduction/Adduction"
                elif i==2 :
                    title=key_left[1:]+ " Rotation"
